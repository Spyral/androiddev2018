...
import vn.edu.usth.irc.Command.CommandParser;
import vn.edu.usth.irc.IRCChat;
...
import vn.edu.usth.irc.Model.ServerInfo;
import vn.edu.usth.irc.Model.Status;
import vn.edu.usth.irc.R;
import vn.edu.usth.irc.Model.Settings;
...
public class IRCConnection extends IRCClient
{
    ...
    {
        this.server = IRCChat.getInstance().getServerById(serverId);
...
//    }
    ...
    protected void onJoin(String target, String sender, String login, String hostname)
    {
        if (sender.equalsIgnoreCase(getNick()) && server.getConversation(target) == null) {
            ...
            Chat conversation = new Channel(target);
            server.addConversation(conversation);
            ...
            service.sendBroadcast(intent);
        } else if (service.getSettings().show) {
